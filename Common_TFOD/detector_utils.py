...
def draw_box(img,distance,height ,coll,midpoints,close,num_face_detect,scores,ID):
    ...
    for i in range(num_face_detect):
        if i in close:
            ...
            cv2.line(img=img,pt1=(660,620),pt2=midpoints[i], color=(0, 0, 255), thickness=2, lineType=8, shift=0)
            cv2.putText(img, 'Object ' + str(i) + ': ' + ID[i], (int(p1[0]), int(p1[1]) - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            cv2.putText(img, 'Distance :' + str(round(distance[i],2)), (int(p1[0]), int(p1[1]) + height[i]+20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            ...
            cv2.putText(img, "ALERT", (posii, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
...
def get_distance(midpoints,height,num_detect):
    dist = []
    for i in range(num_detect): #(680, 440)
        x = midpoints[i][0]- 680
        y = midpoints[i][1]- 440
    ...
    return dist
#
def get_closest(distance,mid,num):
    p1= set()
    for i in range(num):
        if ((distance[i]<220)&(280<=mid[i][0]<=980)&(mid[i][1]>360)):#&(mid[i][1]>440)
            p1.add(i)
